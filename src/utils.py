...
def extract_features(waveform, feat_config, sr=22050):
    """
    Dispatcher: chiama l'estrattore corretto in base a feat_config['type'].

    feat_config : dizionario dalla sezione config['features']
    ...
    if t == 'cqt':
        return extract_cqt(
            ...
            n_bins=feat_config.get('cqt_bins', 84),
        )
    elif t == 'stft':
        return extract_stft(
            waveform, sr=sr, hop_length=hop_length,
            n_fft=feat_config.get('stft_n_fft', 2048),
            fmin=feat_config.get('stft_fmin', 55.0),
            fmax=feat_config.get('stft_fmax', 4200.0),
        )
    elif t == 'mel':
        return extract_mel(
            waveform, sr=sr, hop_length=hop_length,
            n_fft=feat_config.get('mel_n_fft', 2048),
            n_mels=feat_config.get('mel_n_mels', 128),
        )
    else:
        raise ValueError(f"Feature type sconosciuto: '{t}'. Usa 'cqt', 'stft' o 'mel'.")


def get_input_features(feat_config, sr=22050):
    """
    ...
    if t == 'cqt':
        return feat_config.get('cqt_bins', 84)
        ...
        return feat_config.get('mel_n_mels', 128)
