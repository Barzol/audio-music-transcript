...
#
# Fix rispetto alla Phase 2:
#   - plot_loss_curve accetta ora val_losses opzionale e plotta entrambe le curve
...
def plot_loss_curve(train_losses, val_losses=None, save=True):
    """
    Plotta la curva di training loss (e opzionalmente quella di validation).

    Args:
        train_losses (list): loss media per epoch sul train set
        val_losses   (list): loss media per epoch sul val set (opzionale)
        save         (bool): se True salva in plots/loss_curve.png
    """
    ...
    if val_losses and len(val_losses) == len(train_losses):
        ax.plot(epochs, val_losses,
                color='darkorange', linewidth=2, label='Val Loss')
        best_epoch = int(np.argmin(val_losses)) + 1
        best_loss  = min(val_losses)
        ref_losses = val_losses
    else:
        best_epoch = int(np.argmin(train_losses)) + 1
        best_loss  = min(train_losses)
        ref_losses = train_losses
...
def plot_precision_recall_threshold(all_probs, all_labels, save=True):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(thresholds, precision, color='royalblue', linewidth=2, label='Precision')
...
def plot_piano_roll(labels, preds, track_id="sample", threshold=0.3, save=True):
    ...
    ax.imshow(rgb, aspect='auto', origin='lower',
              extent=[0, time_frames, 0, num_notes])
...
def plot_prob_distribution(all_probs, all_labels, save=True):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.hist(inactive_probs, bins=50, color='steelblue',
            label='Inactive notes', density=True, alpha=0.5)
    if len(active_probs) > 0:
        ax.hist(active_probs, bins=50, color='darkorange',
                label='Active notes', density=True, alpha=0.5)

    ax.set_title('Probability distribution: Active vs Inactive',
                 fontsize=14, fontweight='bold')
