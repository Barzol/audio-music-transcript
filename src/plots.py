...
def plot_loss_curve(train_losses, val_losses=None, save=True):
    ...
    Args:
        ...
        val_losses   (list of float): average validation loss per epoch (optional)
        save         (bool)         : if True, saves to plots/loss_curve.png
    '''
...
def plot_piano_roll(labels, preds, track_id="sample", threshold=0.3, save=True):
    '''
    Draws a colour-coded piano roll:
      - Purple  : True Positive  (note correctly detected)
      - Blue    : False Negative (note missed)
      - Red     : False Positive (spurious detection)
    '''
...
def plot_confusion_per_note(all_labels, all_preds, threshold=0.3, save=True):
    '''
    For each of the 84 piano notes shows TP / FP / FN counts.
...
def plot_prob_distribution(all_probs, all_labels, save=True):
    '''
    Histogram of sigmoid probabilities for active vs inactive notes.
